# Expert module - Calibration algorithms {#sec-calibration_algorithms}

## Introduction to the calibration module {#sec-calibration_algorithms_intro}
...
## Objective function {#sec-calibration_algorithms_of}

A flexible objective function (OF) has been developed for the module of calibration aiming to be adapted to the user's requirements. The indicators presented in Chapter 4 are used in this OF, each one weighted with a value defined by the user (@tbl-weights_indicators_of).
...
## Shuffled Complex Evoluation -- University of Arizona {#sec-calibration_algorithms_sce_ua}
...
A general description of the steps of the SCE-UA method is given below (Duan et al., 1994) and illustrated in @fig-flowchart_sce.
...
Evolve each complex: Evolve each complex independently by taking NSPL evolution steps, according to the Competitive Complex Evolution (CCE) algorithm. @fig-sce_ua_steps illustrates how each evolution step is taken.
...
![Flow chart of the shuffled complex evolution method (from Duan et al., 1993), with *V=n*, *NGS=p*, *NPG=m* and *NPT=s*](figures/fig-flowchart_sce.ps){#fig-flowchart_sce}

The SCE-UA method is explained in @fig-sce_ua_method and @fig-sce_ua_steps for a two dimensional case (Duan et al., 1994). The contour lines in @fig-sce_ua_method and @fig-sce_ua_steps represent a function surface having a global optimum located at (4,2) and a local optimum located at (1,2). @fig-sce_ua_method (a) shows that a sample population containing NPT (=10) points is divided into NGS (=2) complexes. Each complex contains NPG (=5) points which are marked by **•** and \* respectively. @fig-sce_ua_method (b) shows the locations of the points in the two independently evolved complexes at the end of the first cycle of evolution. It can be seen that one complex (marked by \*) is converging towards the local optimum, while the other (marked by **•**) is converging toward the global optimum. The two evolved complexes are shuffled according to step 5. @fig-sce_ua_method (c) displays the new membership of the two evolved complexes after shuffling.
...
## Uniform Adaptive Monte Carlo {#sec-calibration_algorithms_uamc}
...
The algorithm randomly launches a collection of simulations (block) and finds the better results in the solution space. Afterwards, the solution space is adjusted and a new group of simulations starts. The process is repeated until the optimization converges to the best set of parameters (@fig-flowchart_uamc).
...
![Flow chart of the UAMC algorithm](figures/fig-flowchart_uamc.ps){#fig-flowchart_uamc}

## Coupled Latin Hypercube and Rosenbrock {#sec-calibration_algorithms_rosenb}

### Model architecture

The Coupled Latin Hypercube (McKay et al., 1979) and the Rosenbrock algorithm (Rosenbrock, 1960), called hereafter CLHR, generates a powerful tool for optimization of complex problems. This combined algorithm can discretize a wide domain and then narrow your search to smaller sectors (@fig-operation_clhr). Scanning of the space of possible solutions is performed by the Latin Hypercube. This algorithm allows pseudo-statistical sampling conditioned by the previous calculated solutions. The Latin hypercube is an evolution of Monte Carlo method, with more homogeneous samples achieved with fewer samples. An important advantage of this method is that the dimension of the problem is defined by the division of the latin hypercube and not by the number of parameters.
...
![Illustration of the operation of the CLHR algorithm](figures/fig-operation_clhr.ps){#fig-operation_clhr}
...
Rank points: The results obtained in the first step are ordered (@fig-operation_hypercube). The best results from the Latin hypercube algorithm will serve as
starting points to launch the Rosenbrock algorithm.

![Illustration of the operation of the Latin Hypercube algorithm](figures/fig-operation_hypercube.ps){#fig-operation_hypercube}
...
![Illustration of the operation of the Rosenbrock algorithm](figures/fig-operation_rosenbrock.ps){#fig-operation_rosenbrock}
...
The *STEPROS* parameter indicates the subdivisions for each parameter's range. It is used to calculate value variations in each of the parameters (*∆~i~*) to be studied, as presented in equation @eq-CAL2.
